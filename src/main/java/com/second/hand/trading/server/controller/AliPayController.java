...
import com.second.hand.trading.server.model.AliPay;
	...
	public String payNotify(HttpServletRequest request) throws Exception {
		if (request.getParameter("trade_status").equals("TRADE_SUCCESS")) {
			...
			if (checkSignature) {
				...
				OrderModel order = orderService.getOrder(Long.valueOf(outTradeNo));
				order.setOrderStatus((byte)1);
				order.setPaymentWay("支付宝");
				order.setPaymentStatus((byte)1);
				DateTime gmt_payment = DateUtil.parse(params.get("gmt_payment"), "yyyy-MM-dd HH:mm:ss");
				order.setPaymentTime(gmt_payment);
				orderService.updateOrder(order);
//				orderService.getOrder(outTradeNo)
//				// 查询订单
